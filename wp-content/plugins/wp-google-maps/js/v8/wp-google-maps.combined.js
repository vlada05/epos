...
jQuery(function($) {
	...
	WPGMZA.StoreLocator = function(map, element)
	{
		...
		this.map.on("init", function(event) {
			...
			if(self.map.settings.store_locator_style == "modern")
...
jQuery(function($) {
	...
	WPGMZA.GoogleMap.prototype.loadGoogleMap = function()
	{
		...
		if(this.settings.transport_layer == 2 || this.settings.transport == 1)
...
jQuery(function($) {
	...
	WPGMZA.GooglePolyline.prototype.getScalarProperties = function() {
		var props = WPGMZA.Feature.prototype.getScalarProperties.apply(this, arguments);
		...
		for(var src in map) {
			if(src in props && !empty(props[src]))
				props[map[src]] = props[src];
		}
